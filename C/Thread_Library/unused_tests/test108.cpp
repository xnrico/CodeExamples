...
// MUTEX TEST 108, if WaitingQueue is static, this should be a problem, similart to Test 107
...
void parent(void *arg) {
    ...
    std::cout << "Parent about to unlock m2\n";
    m2.unlock();
    assert_interrupts_enabled();
    thread::yield(); // Go back to child, now if queue is static, child 0 with m1 will do work instead of child1 in certain interleavings

    assert_interrupts_enabled();

    std::cout << "Parent returning\n";
    // Also returning without unlocking m1 -> but this should be fine
}
