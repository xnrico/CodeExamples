...
void service_func(void* arg) {
    ...
    while (1) {
        ...
        // std::cout << "[SERVICER] Num of Living threads: " << num_living_threads << ";
        // Size of QUEUE: " << queue.size() << "/" << max_disk_queue << std::endl;
        // std::cout << "[SERVICER] Max Disk Queue: " << max_disk_queue << std::endl;

        while (
            (num_living_threads > max_disk_queue && (int)queue.size() < max_disk_queue) ||
            (num_living_threads <= max_disk_queue &&
             (int)queue.size() < num_living_threads)) {
        ...
        // print_service((unsigned int)queue[service_idx]->requester_id, (unsigned
        // int)queue[service_idx]->track);
        std::cout << "Servicing requester id "
                  << (unsigned int)queue[service_idx]->requester_id << " at track "
                  << (unsigned int)queue[service_idx]->track << "\n";
        ...
        // std::cout << "[SERVICER] " << serviced_requests << "/" << total_requests << "
        // serviced...\n"; std::cout << "********************SERVICER
        // FINISHED********************\n";
...
void request_func(void* arg) {
    ...
    while (1) {
        ...
        while ((completed[req->requester_id] == false) ||
               (!req->requests.empty() && ((num_living_threads >= max_disk_queue &&
                                            (int)queue.size() >= max_disk_queue) ||
                                           (num_living_threads < max_disk_queue &&
                                            (int)queue.size() >= num_living_threads)))) {
            // std::cout << "Requester [" << req->requester_id << "] waiting to issue
            // request, " << req->requests.size() << " remaining\n";
            cv2.wait(mu1);
        }

        if (req->requests.size() > 0) {
            queue.push_back(new request{req->requests.front(), req->requester_id});
            // print_request((unsigned int)req->requester_id, (unsigned
            // int)req->requests.front());
            std::cout << "Requester id " << (unsigned int)req->requester_id
                      << " issues request at track "
                      << (unsigned int)req->requests.front() << "\n";
...
int main() {
    ...
    int argcs = 9;
    const char* argvs[] = {"./test50", "null",     "disk.in0", "disk.in1", "disk.in2",
                           "disk.in3", "disk.in4", "disk.in5", "disk.in6"};

    for (int i = argcs; i > 2; --i) {
        std::ifstream file(argvs[2 + num_requester]);
        if (!file) {
            std::cerr << "Error opening file: " << argvs[2 + num_requester] << std::endl;
            return 1;
        }
        ++num_requester;
