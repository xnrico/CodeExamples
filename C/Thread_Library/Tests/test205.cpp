...
void child0(void *arg) {
    ...
    while (unfinishedChild1 != 0) {
        ...
        std::cout << "[Child " << *((size_t *)arg)
                  << "] completed work and wakes up all waiting children\n";
        m.unlock();
    }

    assert_interrupts_enabled();
    std::cout << "ALL CHILDREN FINSIHED! [Child " << *((size_t *)arg)
              << "] unlocked sucessfully, deleting then returning\n";
    delete (size_t *)arg;
}
