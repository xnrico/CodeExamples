...
static ili9488_status_t ili9488_driver_read_spi_error			(void);
static ili9488_status_t ili9488_driver_read_display_status		(void);
static ili9488_status_t ili9488_driver_set_display_normal_mode	(void);
static ili9488_status_t ili9488_driver_read_power_mode			(void);
static ili9488_status_t ili9488_driver_read_madctl				(void);
...
//		   argsNum = val & 0x7F;
//		   if(val & 0x80) {
//			   time = (*addr)*255 + (*addr)+1;
//			   addr += 2;
//		   } else {
//			   time = 0;
//		   }
//		   ili9488_low_if_write_register(cmd, addr, argsNum);
...
//		status |= ili9488_driver_set_all_pixels(eILI9488_ALL_PIXELS_OFF);
//		status |= ili9488_driver_set_all_pixels(eILI9488_ALL_PIXELS_ON);
//		status |= ili9488_driver_set_display_normal_mode();
//
//		status |= ili9488_driver_read_spi_error();
//
//		status |= ili9488_driver_read_display_status();
//
//		status |= ili9488_driver_read_power_mode();
//
//		status |= ili9488_driver_read_madctl();
...
{
	ili9488_status_t status = eILI9488_OK;
	const uint8_t gamma[15] = {0x00, 0x03, 0x09, 0x08, 0x16, 0x0A, 0x3F, 0x78, 0x4C, 0x09, 0x0A, 0x08, 0x16, 0x1A, 0x0F};
	//"\x00\x03\x09\x08\x16\x0A\x3F\x78\x4C\x09\x0A\x08\x16\x1A\x0F";
...
{
	ili9488_status_t status = eILI9488_OK;
	const uint8_t gamma[15] = {0x00, 0x16, 0x19, 0x03, 0x0F, 0x05, 0x32, 0x45, 0x46, 0x04, 0x0E, 0x0D, 0x35, 0x37, 0x0F};
	// "\x00\x16\x19\x03\x0F\x05\x32\x45\x46\x04\x0E\x0D\x35\x37\x0F";
...
ili9488_status_t ili9488_driver_read_pixel_format(ili9488_pixel_format_t * const p_format)
{
	...
	// Read register
	status = ili9488_low_if_read_register( eILI9488_READ_PF_CMD, (uint8_t*) &rx_buf, 2U );
...
ili9488_status_t ili9488_driver_set_cursor(const uint16_t col_s, const uint16_t col_e, const uint16_t page_s, const uint16_t page_e)
{
	...
	else
	{
		...
		status |= ili9488_low_if_write_register( eILI9488_SET_COL_ADDR_CMD, (uint8_t*) &baundary[0], 4U );
		...
		status |= ili9488_low_if_write_register( eILI9488_SET_PAGE_ADDR_CMD, (uint8_t*) &baundary[0], 4U );
	}
...
ili9488_status_t ili9488_driver_read_memory(uint8_t * const p_mem, const uint32_t size)
{
	...
	status = ili9488_low_if_read_memory(p_mem, size );
...
static ili9488_status_t ili9488_driver_read_spi_error(void) {
	...
	ili9488_low_if_read_register(eILI9488_READ_SPIERR_CMD, (uint8_t *) &err, 2);
...
static ili9488_status_t ili9488_driver_read_display_status(void) {
	...
	ili9488_low_if_read_register(eILI9488_READ_STATUS_CMD, (uint8_t *) data, 6);


	ili9488_pixel_format_t pf;
	ili9488_driver_read_pixel_format(&pf);

	if (pf != eILI9488_PIXEL_FORMAT_18_BIT) {
		status = eILI9488_ERROR;
	}

	printf("Status: %d %d %d %d", data[0], data[1], data[2], data[3]);
...
static ili9488_status_t ili9488_driver_set_display_normal_mode(void) {
	ili9488_low_if_write_register(eILI9488_NORMAL_MODE_CMD, NULL, 0);
	return eILI9488_OK;
}
...
static ili9488_status_t ili9488_driver_read_power_mode(void) {
	...
	status = ili9488_low_if_read_register(eILI9488_READ_PWR_STATUS_CMD, (uint8_t *) data, 2);

	printf("Status: %d %d", data[0], data[1]);
...
static ili9488_status_t ili9488_driver_read_madctl(void) {
	...
	status = ili9488_low_if_read_register(eILI9488_READ_MADCTL_CMD, (uint8_t *)  data, 2);
