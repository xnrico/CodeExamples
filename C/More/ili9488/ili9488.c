...
// Function prototypes
////////////////////////////////////////////////////////////////////////////////
ili9488_status_t ili9488_read_memory(uint8_t *data, uint32_t size);
...
ili9488_status_t ili9488_draw_bitmap(uint8_t *bm) {
	//iterate through every pixel. Each is 2 bytes
	//for(uint32_t i = 0; i < (480*320)-1; i+=2) {
		//ili9488_driver_set_pixel(i%480+1, i/, color );
	//}
	ili9488_status_t status = eILI9488_OK;
	uint32_t size = 480*320*3;
	status |= ili9488_driver_set_cursor( 0, ILI9488_DISPLAY_SIZE_COLUMN, 0, ILI9488_DISPLAY_SIZE_PAGE);
	ili9488_driver_write_memory(bm, size);

	return status;
}

ili9488_status_t ili9488_read_memory(uint8_t *data, uint32_t size) {
	ili9488_status_t status = ili9488_driver_read_memory(data, size);
	return status;
}
