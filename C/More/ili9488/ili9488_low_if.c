...
ili9488_status_t ili9488_low_if_read_memory(uint8_t * const p_mem, const uint32_t size)
{
			ili9488_status_t 	status 	= eILI9488_OK;
	const 	uint8_t 		 	cmd 	= eILI9488_READ_MEM_CMD;
			uint32_t 			i		= 0UL;
	...
	else
	{
		...
		if ( size > 0 )
		{
			...
			ILI9488_LOW_IF_DC_DATA();
			uint8_t dummy = 0;
			ili9488_if_spi_receive((uint8_t*) &dummy, 1U);
			for ( i = 0; i < size; i++ )
			{
				ili9488_if_spi_receive((uint8_t*) p_mem, 3U );
			}
		}
	}
