...
def print_detections(detections, coordinates=False):
    ...
    for label, confidence, bbox in detections:
        x, y, w, h = bbox
        if coordinates:
            with open('cam0.txt', 'a') as f:
                contents = ("{}: {}%    (left_x: {:.0f}   top_y:  {:.0f}   width:   {:.0f}   height:  {:.0f})".format(label, confidence, x, y, w, h) + "\n")
                f.write(contents)
            f.close()
            print("{}: {}%    (left_xxx: {:.0f}   top_y:  {:.0f}   width:   {:.0f}   height:  {:.0f})".format(label, confidence, x, y, w, h))
        else:
