...
def nmpc_controller(kappa_table = None):
    ...
    for k in range(N):
        ...
        cons_ineq.append(u[0, k]*x[0, k] - param['Peng'])
        cons_ineq.append(1 - ((x[3, k] - 500)/10.5)**2 - (x[4, k]/10.5)**2)
    ...
    J = 0.0
    J = J + 10 * x[4, -1]**2 # Y -> 0
    J = J + 10 * x[5, -1]**2 # Yaw -> 0
    J = J + 5 * (1e2 - x[0, -1])**2 # Ux -> 200 (Large Speed)
    J = J + 1e-2 * (2e3 - x[3, -1])**2 # X -> 2000 (Far Away)
    
    ## road tracking 
    for k in range(N):
        J = J + 5 * x[4, k]**2 # Y => 0
        J = J + 5 * x[5, k]**2 # Yaw -> 0
        J = J + 10 * (1e2 - x[0, k])**2 # Ux -> 200 (Large Speed)
        J = J + 2.5e-3 * (2e3 - x[3, k])**2 # X -> 20000 (Far Away)
    ...
    ctrl_ub  = np.array([1e8, param['delta_max']])
    ctrl_lb  = np.array([-1e8, -param['delta_max']])
