...
def nmpc_controller():
    ...
    beta = ca.atan(L_r/(L_f+L_r)*ca.atan(u_model[1]))
    ...
    P = p * ((v_des - x_model[3])**2 + x_model[1]**2)
    L = q*3 * (v_des - x_model[3])**2 + q * xdot[1]**2 + q*0.01 * xdot[2]**2 + q*5 * x_model[1]**2

    Fun_cost_terminal = ca.Function('P', [x_model, par], [P])
    Fun_cost_running = ca.Function('Q', [x_model, u_model, par], [L])

    # state and control constraints
    state_ub = np.array([ 1e4,  3,  3.14,  200])
    state_lb = np.array([ -1e4,  -1,  -3.14,  -200])
    ...
    for k in range(N):
        ...
        #### lane keeping ####
        cons_state.append(x[1, k] - 2.95)
        cons_state.append(-x[1, k] - 1)

        #### steering rate ####
        if k >= 1:
            d_delta = (u[1, k] - u[1, k-1]) / h
            cons_state.append(d_delta - 0.55)
            cons_state.append(-d_delta - 0.55)
        else:
            d_delta = u[1, 0] - delta_last 
            cons_state.append(d_delta - 0.55) 
            cons_state.append(-d_delta - 0.55) 
